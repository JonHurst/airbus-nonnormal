...
The autopilot can fly the escape manoeuvre as long as required \ac{AOA} is less
than $\alpha$\textsubscript{prot}. If the autopilot is not engaged, follow the
\ac{SRS} orders on the \ac{FD}s. If the \ac{FD}s are not available, initially
pitch up to 17.5\textdegree, then increase as required.
...
effect. Once clear of the windshear, however, \ac{TOGA} lock will be active, requiring
the autothrust to be disconnected to avoid an overspeed. This is most naturally
achieved by pressing the instinctive disconnect \ac{PB} on the thrust levers
then flying manual thrust until the situation has sufficiently stabilised to
